...
export default function CertificatesPage() {
  ...
  const handleDownloadPDF = () => {
    setIsPrinting(true);
    // Give state time to update to hide UI elements if needed, then trigger print
    setTimeout(() => {
      window.print();
      setIsPrinting(false);
    }, 100);
  ...
  const handleLinkedInShare = (courseName: string) => {
    const shareUrl = `https://www.linkedin.com/feed/?shareActive=true&text=I%20just%20earned%20my%20certificate%20in%20${encodeURIComponent(courseName)}%20on%20EduVerse!`;
  ...
  return (
    <div style={{ backgroundColor: '#f3f4f6', minHeight: '100vh', padding: '100px 20px 80px' }}>
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isPrinting ? '1fr' : 'repeat(auto-fill, minmax(350px, 1fr))', gap: '32px' }}>
          {certificates.map((cert) => (
            <div key={cert.id} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', border: '1px solid rgba(108,92,231,0.08)', boxShadow: '0 10px 30px rgba(0,0,0,0.02)', transition: 'transform 0.2s', cursor: 'pointer', pageBreakInside: 'avoid', marginBottom: isPrinting ? '40px' : '0' }} onMouseEnter={(e) => { if(!isPrinting) e.currentTarget.style.transform = 'translateY(-4px)'}} onMouseLeave={(e) => { if(!isPrinting) e.currentTarget.style.transform = 'translateY(0)'}}>
              
              {/* Certificate Preview Top */}
              <div style={{ height: isPrinting ? '400px' : '180px', background: `linear-gradient(135deg, ${cert.color}, #1a1a2e)`, position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px', textAlign: 'center', borderBottom: '4px solid #fff' }}>
                <i className="fas fa-certificate" style={{ fontSize: isPrinting ? '5rem' : '3rem', color: 'rgba(255,255,255,0.2)', position: 'absolute', top: '20px', right: '20px' }}></i>
                <h2 style={{ color: 'white', fontFamily: 'Outfit, sans-serif', fontSize: isPrinting ? '3rem' : '1.4rem', margin: '0 0 8px 0', textShadow: '0 2px 4px rgba(0,0,0,0.2)' }}>{cert.course}</h2>
                <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: isPrinting ? '1.5rem' : '0.9rem', margin: 0 }}>Certificate of Completion</p>
                {isPrinting && <p style={{ color: 'white', fontSize: '1.2rem', marginTop: '40px' }}>Awarded to you for successful completion.</p>}
              </div>
              ...
              <div style={{ padding: '24px' }}>
                ...
                {!isPrinting && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                    <button onClick={handleDownloadPDF} style={{ padding: '10px', background: 'rgba(108,92,231,0.1)', color: '#6c5ce7', border: 'none', borderRadius: '8px', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'background 0.2s' }} onMouseEnter={(e) => e.currentTarget.style.background = 'rgba(108,92,231,0.2)'} onMouseLeave={(e) => e.currentTarget.style.background = 'rgba(108,92,231,0.1)'}>
                      <i className="fas fa-download"></i> PDF
                    </button>
                    <button onClick={() => handleLinkedInShare(cert.course)} style={{ padding: '10px', background: 'white', color: '#0077b5', border: '1px solid #0077b5', borderRadius: '8px', fontSize: '0.95rem', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', transition: 'all 0.2s' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#0077b5'; e.currentTarget.style.color = 'white'; }} onMouseLeave={(e) => { e.currentTarget.style.background = 'white'; e.currentTarget.style.color = '#0077b5'; }}>
